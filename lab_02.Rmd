...
running R code `unzip("organize_me.zip")`. Do this in your console so you only unzip the folder once and not each time you knit the Rmd file.
...
x[x %in% letters]
numbs <- x[!(x %in% letters)]
numbs[as.numeric(numbs) <0]
numbs[as.numeric(numbs) %% 3 == 0]
evens = numbs[as.numeric(numbs) %% 2 == 0]

rm_evens = x[!(x %in% evens)]
rm_evens
```
